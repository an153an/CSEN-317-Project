...
"""
CIFAR-10 FedAvg Training
Same as fedavg_simple.py but using CIFAR-10 dataset
"""
import torch
...
def split_data_non_iid(dataset, num_clients=5):
    """Split dataset into non-IID parts"""
...
def federated_training(num_rounds=50, num_clients=5, local_epochs=5):
    ...
    # Initialize models
    global_model = SimpleNN(input_size=3072, hidden_size=256).to(device)
    client_models = [SimpleNN(input_size=3072, hidden_size=256).to(device) for _ in range(num_clients)]
    
    client_train_accs = [[] for _ in range(num_clients)]
    server_test_accs = []
    
    print("\nStarting FedAvg (CIFAR-10)")
    print("="*60)
...
def run_experiment(num_repeats=5, num_rounds=50):
    ...
    all_client_accs, all_server_accs, final_accs = [], [], []
...
def plot_results(all_client_accs, all_server_accs):
    ...
    ax1.set_title('Client Training (CIFAR-10)', fontsize=14)
    ax1.legend()
    ...
    ax2.set_title('Server Test (CIFAR-10, Avg 5 runs)', fontsize=14)
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('results/fedavg_cifar10.png', dpi=300)
    print("\nPlot saved to results/fedavg_cifar10.png")
    plt.show()
